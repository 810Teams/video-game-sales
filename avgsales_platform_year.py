...
def create_chart(data_frame):
    ...
    data_count = sorted(numpy.array(data_frame.groupby(['Platform', 'Year'], as_index=False).count()[['Platform', 'Year', 'Rank']]).tolist())
    data_sales = sorted(numpy.array(data_frame.groupby(['Platform', 'Year'], as_index=False).sum()[['Platform', 'Year', 'Global_Sales']]).tolist())

    #Data List of Platforms
    data_handhelds = fill_missing([[int(data_count[i][1]), data_sales[i][2]/data_count[i][2]] for i in range(len(data_count)) if data_count[i][0] == 'HandheldGameConsoles' and data_count[i][1] <= 2016])
    data_consoles  = fill_missing([[int(data_count[i][1]), data_sales[i][2]/data_count[i][2]] for i in range(len(data_count)) if data_count[i][0] == 'HomeVideoGameConsoles' and data_count[i][1] <= 2016])
    data_windows   = fill_missing([[int(data_count[i][1]), data_sales[i][2]/data_count[i][2]] for i in range(len(data_count)) if data_count[i][0] == 'MicrosoftWindows' and data_count[i][1] <= 2016])
